...
**Wave 1 batch 1 status: 0.44.0 — orchestrator + missing sub-includes shipped.** `collaboration_panel.html` (orchestrator), `attachment_list.html`, and `quick_info.html` all live in `keel/core/templates/keel/components/` with 15 render tests pinning the contract. Sub-sections opt out by omitting their data kwarg; `collapsed=True` wraps the panel in `<details>` for the Admiralty carve-out (statutory-clock-driven workflow). Wave 2 (Yeoman adoption) can now consume this. Still deferred to Wave 1 batch 2: the `{% collaboration_panel object %}` template tag (DX-phase recommendation — replaces 6-kwarg include with single-arg) and the `preview_collaboration_panel` management command (needs Wave 2's real consumer first).
