"""
Signature workflow orchestration services — shared between harbor and
manifest (phase b of the services.py dedup described in
``keel/signatures/__init__.py``).

Both products' ``signatures`` apps re-export from this module. Their
bespoke ``SigningPacket`` / ``SigningStep`` / ``SignaturePlacement``
models stay product-local (phase e, still deferred); this module
resolves them at runtime via ``apps.get_model('signatures', …)`` so
the same code runs in either deployment.

The compat layer stays product-local for now (phase c) because it
embeds harbor-specific role enums and manifest-specific SignatureRole
model access that don't share cleanly. Services.py imports compat
from the absolute path ``signatures.compat`` — both products register
the app under that label, so this resolves correctly in either process.
"""
import base64
import io
import logging
...
# Compat layer stays product-local; import from the absolute app label
# ``signatures`` rather than keel.signatures so we pick up the running
# product's compat.py (harbor-style or manifest-style).
from signatures.compat import (
    build_absolute_url,
    create_notification,
    get_audit_action,
    log_audit,
...
def _m(name):
    """Resolve a bespoke signatures model from the running product's app.

    The ``signatures`` app label is shared by harbor's and manifest's
    signatures apps; the model classes live in each product's
    ``signatures/models.py`` (phase e of the extraction will consolidate
    those via ``SeparateDatabaseAndState``; until then we resolve at
    runtime).
    """
    return apps.get_model('signatures', name)
...
def initiate_packet(flow, title, initiated_by, signer_assignments,
    ...
    """
    SigningPacket, SigningStep = _m("SigningPacket"), _m("SigningStep")
...
def _complete_packet(packet):
    ...
    SigningPacket = _m("SigningPacket")
...
def generate_signed_pdf(packet):
    ...
    SignaturePlacement, SigningStep = _m("SignaturePlacement"), _m("SigningStep")
