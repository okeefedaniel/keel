/**
 * DockLabs Design System — "Warm Harbor" v2 JavaScript
 * Used by: All DockLabs applications
 * Requires: Bootstrap 5.3+
 *
 * Include via Keel's static files:
 *   <script src="{% static 'js/docklabs-v2.js' %}"></script>
 */
(function () {
  'use strict';
  ...
  // =========================================================================
  // 11. Sidebar — toggle, tree state, persistence
  // =========================================================================
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (localStorage.getItem('dl-sidebar-collapsed') === 'true') {
      sidebar.classList.add('collapsed');
    }

    // Restore tree expansion state
    var product = document.body.getAttribute('data-product') || 'default';
    var treeStateKey = 'dl-tree-state-' + product;
    try {
      var treeState = JSON.parse(localStorage.getItem(treeStateKey) || '{}');
      Object.keys(treeState).forEach(function (id) {
        var el = document.getElementById(id);
        var toggle = document.getElementById(id.replace('-tree', '-toggle'));
        if (el && treeState[id]) {
  ...
  /**
   * Toggle a tree section's expand/collapse state.
   * @param {string} id - The ID of the .sb-children element
   */
  window.dlToggleTree = function (id) {
    var el = document.getElementById(id);
    if (!el) return;
    var toggleId = id.replace('-tree', '-toggle');
    ...
    try {
      var treeState = JSON.parse(localStorage.getItem(treeStateKey) || '{}');
      treeState[id] = !el.classList.contains('collapsed');
      localStorage.setItem(treeStateKey, JSON.stringify(treeState));
    } catch (e) { /* ignore */ }
  };

  // =========================================================================
  // 12. Keyboard shortcut: Cmd/Ctrl+\ to toggle sidebar
  // =========================================================================
