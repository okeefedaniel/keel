...
- **Railway "Wait for CI" gate — silent SKIPPED deploys.** All 9 services are wired to GitHub source with Railway's Source → Check Suites toggle ("Wait for CI") enabled (helm, manifest, harbor, admiralty, beacon, yeoman, lookout). When the GitHub Actions check suite fails on a commit, Railway records the deploy with `status=SKIPPED` and `skippedReason="CI check suite failed"` — no build logs, no error in the dashboard, the previous SUCCESS image keeps serving. `railway redeploy` redeploys the last successful image (still stale); only `railway up` (uploads tarball, bypasses GitHub gate) ships the new code. **Diagnose:** `railway deployment list --service <name> --json | jq '.[].meta.skippedReason'` — if the reason is "CI check suite failed", fix the failing CI step (it's almost always Bandit B602 on `subprocess.run(..., shell=True)` in `startup.py`, or a keel pin pointing at a rebased orphan SHA, or a Django app with a broken `import *` re-export). Purser and bounty have the toggle off and ship even when CI is red — that's why they look unaffected.
