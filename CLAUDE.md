...
- **Django template comments: `{# … #}` is SINGLE-LINE only.** Django's `{# … #}` short comment tag does NOT span multiple lines — the comment must open and close on the same physical line, or Django renders the would-be comment as literal text in the rendered HTML. For any comment that wraps across lines (rationale notes, multi-sentence explanations, ASCII-art separators, anything that touches a newline), use `{% comment %} … {% endcomment %}`. This bit demo-keel's login page on 2026-05-11 when a multi-line `{# … #}` rationale block rendered visibly above the demo-login buttons. Same rule applies whether you're writing a brand-new template or pasting a code-style block comment into Django HTML.
