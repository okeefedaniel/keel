...
- **Suite-wide keel pin state (2026-05-13).** All 9 products are pinned to `keel @ v0.38.x` and serve the canonical `.dl-status-dot` rule out of `docklabs-v2.css`. Six products on v0.38.0 (Admiralty, Beacon, Harbor, Helm, Manifest, Purser), three on v0.38.1 (Bounty, Lookout, Yeoman — the three that hit the pip-cache trap below and needed a fresh-wheel forcing function). Verification check for any future audit: `curl https://demo-<product>.docklabs.ai/static/css/docklabs-v2.*.css | grep -c dl-status-dot` should return **17**. A 0 indicates the same stale-wheel regression.
...
- **Three audit carry-forwards.** (1) CI smoke test that asserts `.dl-status-dot` is present in deployed `docklabs-v2.css` — would have caught the Bounty/Yeoman stale-wheel regressions at deploy time rather than via DOM inspection. (2) Yeoman's legacy `_status_badge.html` inclusion template is orphaned but kept; clean up in a follow-up. (3) Local-only `claude/audit-feed-mount` branches exist on 7 product repos; they depend on `keel.feed.audit_feed_view` which lives only on a keel claude/worktree branch (`claude/xenodochial-shtern-4c2525`), not in any keel release tag. Both sides need to land together — DO NOT merge the product-side branches alone, they would break at boot with `ImportError`.
