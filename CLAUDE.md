...
- **Django template comments: `{# … #}` is SINGLE-LINE only — enforced by CI.** If the comment touches a newline, Django renders it as literal text in the HTML. Use one of:
  ...
  Never paste a code-style `/* multi-line */` block into a Django template as `{# … #}`. Bounty's CI runs a grep guard (`.github/workflows/ci.yml` → "Guard against multi-line Django `{# … #}` comments") that fails the build on any newline-spanning `{# … #}` in `templates/**/*.html`; clone the same step into any other keel-consumer's CI when adopting this convention. Prior incidents: demo-keel login page (2026-05-11), bounty tracked-opportunity detail page (2026-05-17).
