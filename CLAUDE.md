...
- **Adding a new `ProductAccess` field that should reach products: use `SYNCED_FIELDS` (keel ≥ 0.38.0).** `keel.accounts.models.SYNCED_FIELDS` is the single source of truth for which `ProductAccess` fields cross the OIDC boundary. Append a `SyncedField(field, claim, shape)` entry, add the matching claim name to `DOCKLABS_CUSTOM_CLAIMS` and `oidc_claim_scope`, and both the validator's `get_additional_claims` and the SSO adapter's `_mirror_product_access` pick it up automatically. The boot-time check `KeelOIDCValidator.validate_claim_scope` raises `ImproperlyConfigured` at app start naming any unwired field. Use `shape='dict'` for string-valued fields (emit `{code: value}`, e.g. `role` → `product_access`) and `shape='list'` for booleans (emit `[codes where flag is True]`, e.g. `is_beta_tester` → `beta_products`). Pre-existing claims that have computed semantics (e.g. `ai_enabled_products` is org-sub × per-user intersection) pass a `builder=` callable that overrides the default per-user query. The mechanism exists because `is_beta_tester` shipped between v0.34 and v0.36 with the model field and `Invitation.accept` wired up but the OIDC boundary entirely missing — the flag had zero observable effect on any product for months before someone noticed.
...
- **Role hierarchy — only `system_admin` bypasses per-record ACL.** `helm_admin`, `agency_admin`, and analogous per-product admin labels are BASELINE `ProductAccess` roles that grant entry to the product but do NOT, on their own, confer cross-project / cross-record visibility. Only `system_admin` (plus Django `is_superuser` / `is_staff`) bypasses per-project / per-record ACLs. Operationally privileged in-product actions (delete others' tasks, remove collaborators, bulk import) require Django staff or `system_admin`, not the per-product admin label. Helm's `tasks` ACL is the reference (see commit `27bbc38`, 2026-05-02 — `fix(tasks): scope helm_admin role to product access, not god mode`); products diverging from this contract should align with Helm. Note that the base `WorkflowEngine._user_has_role` still grants `agency_admin` a per-transition bypass for state machines — that's a separate, broader gate from per-record visibility, and product workflow subclasses should tighten it (as Helm's `ProjectWorkflowEngine` does) when records carry per-instance ACL.
...
- **Three deployment modes.** `keel.core.utils.is_suite_mode()` is the canonical detector. Returns `True` only when `KEEL_OIDC_CLIENT_ID` is set AND `DEMO_MODE` is false. Use it before branching on deployment topology.
  ...
  - **Demo** (`DEMO_MODE=True`): signup + admin work regardless. Demo users are seeded via `seed_keel_users` with `set_unusable_password()` — login is exclusively via the one-click `demo_login_view` buttons (`/demo-login/`), never the standard auth form. See the **Demo authentication — passwordless contract** subsection in Demo Mode for the full contract.
...
- **AutoOIDCLoginMiddleware** skips auto-OIDC redirect in `DEMO_MODE` so demo users can sign in via the one-click role buttons on the local login page (`/demo-login/`) without being bounced through Keel OIDC.
...
### Demo authentication — passwordless contract (keel ≥ 0.20.1)

- **Demo users have no usable password.** `seed_keel_users` calls `user.set_unusable_password()` on every run (both create and update paths). There is no shared `DEMO_PASSWORD` env var, no shared password value, and `/accounts/login/` cannot authenticate a demo user — there is no hash to verify against.
- **The only entry path is `keel.core.demo.demo_login_view`** (`/demo-login/` — the one-click "Sign in as <Role>" buttons rendered by `{% demo_login_buttons %}`). The view enforces, in order: (1) `DEMO_MODE=True` or 403, (2) role membership in `get_demo_roles()`, (3) rate limit 10/min, (4) user exists and `is_active`. It then calls `django.contrib.auth.login(request, user, backend='django.contrib.auth.backends.ModelBackend')` directly — `authenticate()` is never called because there is nothing to verify.
- **Why this design.** Pre-0.20.1, demo users had a real Django hash equal to a shared `DEMO_PASSWORD` so the button handler's `authenticate()` call could succeed. That credential was never typed by a real human — only the seed command and the button view consumed it — but it was still a fully-functional Django password, so `/accounts/login/` was a parallel attack surface: anyone who guessed `demo2026!` (the suite-wide hardcoded fallback) bypassed the role-allowlist + rate-limit gates by going through the standard form. The 0.19.0 CSO patch added a boot tripwire (`_validate_demo_password`) that required a non-default `DEMO_PASSWORD` per service, which solved the security problem but turned every demo deploy into a coordination chore and crash-looped 9 demo services on 2026-04-27 when the keel pin propagated before the env vars were set. 0.20.1 replaces the credential entirely: nothing to guess, nothing to set, nothing to forget.
- **Adding new demo-seed commands.** Always `user.set_unusable_password()`, never `user.set_password(...)`. References: `keel.accounts.management.commands.seed_keel_users` (canonical), `keel.accounts.management.commands.ensure_dokadmin` (the `dokadmin` superuser uses the same pattern). Reviewers should reject any product-specific seed command that writes a real password to a demo account — that re-opens the parallel form-login attack surface.
- **Test code logs in via `client.force_login(user)`,** not `client.login(username=..., password=...)`. `force_login` bypasses authentication entirely, which is exactly right for demo users. The `keel.testing` smoke + workflow harnesses follow this pattern; product test suites should too.
- **Migrating a legacy demo DB.** A demo DB seeded under keel 0.19.x carries a real password hash on every demo user. After bumping to keel ≥ 0.20.1, the next `seed_keel_users` run (which fires on demo redeploy via `run_startup()`) resets every demo user to `set_unusable_password()` automatically — no manual step required. `cleanup_demo_users` does the same and is idempotent. Until one of those runs, the historical form-login attack surface lingers for as long as anyone reading an old runbook could guess `demo2026!`.
- **`DEMO_PASSWORD` env var is ignored.** Leaving it set on a Railway service is harmless; stripping it is purely cosmetic. The variable name is no longer referenced anywhere in keel.
...
- **Tabs are for switching views WITHIN a record, not for top-level section navigation.** Use Bootstrap `.nav-tabs` only when you're showing the same record from multiple facets (e.g. an Award detail page with Documents / Comments / History tabs). Never use tabs to navigate between top-level sections of a product (those are sidebar items). Tabs misused for navigation visually break: the active-tab indicator points down at empty space because there's no panel container to attach to, and the tab metaphor ("same data, different view") fights with the actual content ("different sections of the app"). Helm's `/dashboard/` shipped with this exact bug — Today / Across the suite / Stream tabs — and was refactored in keel 0.32.x era to a sidebar-nav `Dashboard` (Inbox) + `Fleet` split. The shared `docklabs-v2.css` ships an underline-only `.nav-tabs` treatment that doesn't pretend to attach to a panel below it, so legitimate in-record tabs render cleanly without a card wrapper.
...
- **Never put an htmx attribute on an anchor whose `href` is where the user should land.** htmx cancels the default action of any anchor it handles (`shouldCancel()` in htmx 2.x returns true for an `<a href>` whose href isn't a bare fragment), so the click fires the `hx-get` / `hx-post` and the navigation never happens — silently, with no console error and no failed request to find in the network tab. When a click must both hit the server and navigate, do the work server-side: point the anchor at a plain `href` for a view that mutates and then 302s to the destination. `href` + `hx-*` on one anchor is only correct when the href is a no-JS fallback for the same content htmx swaps in place (`keel/components/page_tabs.html` is the legitimate shape; `comms/_panel.html` uses `href="#"`, which htmx does not cancel). Shipped as a real bug: keel's notifications inbox carried `href="{{ notification.link }}"` and `hx-post` to mark-read on the same anchor, so every notification click marked the row read and went nowhere — suite-wide, in all 9 products, for as long as the inbox has existed.
- **Django template comments: `{# … #}` is SINGLE-LINE only — enforced by CI.** If the comment touches a newline, Django renders it as literal text in the HTML. Use one of:
  - one physical line of `{# … #}` (preferred for short notes), OR
  - `{% comment %} … {% endcomment %}` (required for any multi-sentence / multi-line rationale).

  Never paste a code-style `/* multi-line */` block into a Django template as `{# … #}`. **The guard is centralized in keel's reusable security workflow** (`keel/.github/workflows/security.yml` → "Guard against multi-line Django `{# … #}` comments"). It walks every `*.html` in the repo, flags any `{#` whose matching `#}` is on a later line, and fails the build. Because that workflow both triggers on push/PR to keel's own `main` AND is consumed by every product via `uses: okeefedaniel/keel/.github/workflows/security.yml@vX`, the single step covers keel's shipped templates and all 9 products' templates with no per-repo wiring. Do NOT re-add a product-local grep step — there's one guard now, by design. Prior incidents: demo-keel login page (2026-05-11), bounty tracked-opportunity detail page (2026-05-17), keel notifications inbox leaked one to production (2026-06-23) — the last slipped through precisely because the guard had never actually been wired anywhere, including the Bounty CI this note once claimed ran it.
- **`{% include "…" with k=v %}` kwargs must be on ONE physical line.** Django's lexer accepts the inline-include syntax only when the entire `{% ... %}` tag opens and closes on the same line. Multi-line forms like `{% include "x.html" with \n  a=1\n  b=2\n%}` parse without raising an error but the whole `{% include %}` block renders as literal text in the HTML output — silent failure, no traceback. Adopting the shared `keel/components/` partials means passing 3–5 kwargs per include; keep them on one line even when it pushes well past 80 columns, or split into per-call `{% with ... %}{% include ... %}{% endwith %}` blocks if line length is unbearable. Caught 2026-05-16 during yeoman Wave 2 batch 3 — three of four collaboration-panel sub-includes rendered as raw text until collapsed to single-line. The single-line `attachment_list` include worked fine alongside the broken multi-line ones, which is the diagnostic signal.
- **Status pills — default to text + dot, not filled colored pills.** The standard visual treatment for a status field is `● Label` — a 6-8px colored dot followed by the status label in body text color. Reserve filled / heavily-colored pills for *exception* states the user must act on: URGENT, OVERDUE, NEEDS INFO, BLOCKED, FAILED. Routine workflow states (RECEIVED, SCHEDULED, DRAFT, IN PROGRESS, COMPLETED, MONITORING, RESEARCHING, INTRODUCED, etc.) MUST use the dot+text treatment. A row that paints 8 distinct routine statuses as filled colored pills creates color-soup that obscures the records the user actually needs to attend to — by making every row shout, none of them are heard. Implementation lives in the shared `docklabs-v2.css` as `.dl-status-dot` and `.dl-status-pill` (exception variant). The 2026-05-11 suite design audit identified this as the single highest-impact change across the suite; baseline reference is Dan's stated taste: "I don't like the status tags and their colors — I find those distracting."
- **One pill per row.** On list tables and card grids, at most ONE field per row may render as a pill / badge. Adjacent status-shaped fields (priority, position, location, type, agency code, days-left, etc.) MUST demote to plain muted text — color-soup three-pill rows (Lookout's STATUS + PRIORITY + POSITION, Beacon's STATUS + location-tag + PRIMARY, Bounty's agency + POSTED + DAYS LEFT, Purser's TYPE + per-month status grid) violate this rule. Pick the most actionable field as the pill; everything else is plain text with optional dot prefix. If two fields genuinely both need pill-level emphasis, the design is wrong and one of them should move out of the table row entirely.
- **Detail-page layout reference — Yeoman's invitation detail page.** Every product's detail page MUST be benchmarked against `https://yeoman.docklabs.ai/invitations/<id>/`: serif page title + ≤2 muted sub-pills + one plain-text subtitle line; card sections with icon + right-aligned small-caps label; narrow (~380px) sticky right rail with Activity / Quick Info; header action row of ≤4 labeled buttons (one filled primary) or labeled-primary + ≤2 icon-only outline + `▾` overflow dropdown. As of the 2026-07-21 suite audit every product's detail header conforms to this rule — the former cautionary tales (Beacon's Company-detail cluster and Lookout's 5-control bill-detail header) are both resolved: Beacon via beacon#122, and Lookout's bill detail now ships Follow + a single filled primary + `▾` overflow. When in doubt: open Yeoman's invitation detail page and mirror the proportions.
- **Bootstrap Icons 1.11.3** via CDN.
- **Typography — "Civic Institution" v3 editorial stack (self-hosted WOFF2).** Authenticated app surfaces use **Fraunces** (serif display/headings — `var(--font-display)`), **Instrument Sans** (body — `var(--font-sans)`), and **JetBrains Mono** (data/labels/eyebrows — `var(--font-mono)`). The `@font-face` declarations + the three CSS vars live in `docklabs-v2.css`; `app.html`/`auth.html` preload `fraunces-latin.woff2` + `instrument-sans-latin.woff2` for first paint. **Never hardcode a family in new CSS/inline styles — always reference the vars** so the whole authenticated suite stays on one stack. **Poppins (Google Fonts) is retained intentionally only on two non-authenticated surfaces:** the public/marketing layout (`public.html` + its `.hero-section`/`.stats-bar`/`.landing-section` classes, for CT state brand alignment) and the HTML email templates (web-safe fallback face — `'Poppins','Helvetica Neue',Helvetica,Arial`). It was removed from the authenticated app/auth chrome in v0.56.3 (it was being fetched but never rendered there). Do not reintroduce a suite-wide Poppins `<link>`.
...
- **Click-through:** inbox rows link at `keel_notifications:open` (`/notifications/<pk>/open/`), which marks the notification read server-side and then 302s to `notification.link`; a notification with a blank link falls back to the inbox. It is a GET that mutates state, deliberately — the alternative reintroduces the JavaScript dependency whose failure made every row unclickable (see the htmx anchor rule under UI & Frontend). Consequence worth knowing: `read_at` is evidence of delivery, not proof of attention, because a prefetching browser can set it slightly before a human looks. Cross-product deep links resolve because the redirect allow-list is widened with `keel.core.utils.fleet_product_hosts()` (the hosts declared in `KEEL_FLEET_PRODUCTS`). **Do not hand-roll a second open-redirect check** — pass `extra_hosts=` to `keel.core.utils.safe_redirect_url()` so the guard lives in one place.
...
- **Internal notes:** Extend `keel.core.models.AbstractInternalNote` (provides `is_internal` visibility flag, plus the `mentions` M2M added in 0.42.0). Do not create custom note models without this pattern.
...
### @-mentions on notes (`keel.mentions`, added 0.42.0)
...
- **`@username`** — DockLabs user mention. Fires `note_mentioned` via `keel.notifications.notify()` — in-app + email, user-mutable via the standard preferences UI.
- **`@beacon:contact-slug`** — Beacon contact mention. Best-effort cross-product POST to `/api/v1/intake/contact-mentions/` on Beacon (the existing `is_available()` pattern); Beacon appends a `ContactNote` + `ContactMentionProvenance` row to the contact's record. The external person is **not** notified — this is provenance / relationship-tracking, not communication. Picker shows zero contacts when Beacon isn't configured.

**Per-product integration is 5 steps** (see `keel/mentions/README.md`):
1. Add `keel.mentions` to `INSTALLED_APPS`
2. Include `path('keel/mentions/', include('keel.mentions.urls'))`
3. Inherit `MentionFormMixin` + use `MentionableTextarea` on the comment form's `content` field
...
**Rollback is forward-only.** The M2M through-table and `MentionDelivery` rows are an audit trail; do not `migrate <app> zero` on them. Orphaned tables after a keel-pin rollback are harmless and preserve audit continuity.

**Helm inbox surface.** `keel.mentions.helm_inbox.build_inbox_items(user)` returns the recent unread user mentions in the Helm `UserInbox.items[]` shape — wrap into your product's `/api/v1/helm-feed/inbox/` to surface mentions in the "Awaiting Me" column. Beacon contact mentions are excluded from this surface (contacts aren't Helm users).

**Why:** Harbor's comment system is the reference — government staff need to add internal-only notes that applicants can't see. Every workflow-carrying product needs the same claim / collaborate / diligence primitives, and pinning them in keel prevents drift. The @-mention layer extends the collaboration surface so a note can summon a colleague (in-suite) or annotate a contact (cross-product, Beacon-only in v1) without out-of-band Slack.
...
- **Helm feed pipeline is wired for all 8 products.** `keel.feed` provides the shared framework (`helm_feed_view` decorator + `fetch_product_feed` client). All 8 products expose `/api/v1/helm-feed/` with real-time metrics from live data. Helm's `fetch_feeds` management command pulls data into `CachedFeedSnapshot`. **Deployment:** set `HELM_FEED_API_KEY` as a shared env var on **all 10 Railway services** (8 products + Helm + Keel — Keel needs it for the audit-feed aggregator, see below). In `DEMO_MODE`, auth is bypassed and `seed_helm` provides static demo data as a fallback. Feed endpoint files: `harbor/api/helm_feed.py` (reference), `bounty/api/helm_feed.py`, `beacon/api/helm_feed.py`, `admiralty/foia/helm_feed.py`, `manifest/signatures/helm_feed.py`, `lookout/api/helm_feed.py`, `purser/purser/helm_feed.py`, `yeoman/yeoman/helm_feed.py`.
- **Cross-product audit feed pipeline (keel ≥ 0.40.1).** `keel.feed.audit_feed_view` decorator + `fetch_product_audit` client power Keel's `/audit/` page (left sidebar, staff-only — superuser OR `system_admin` / `agency_admin` `ProductAccess`, **scoped per-product** so an agency_admin of Beacon cannot read Harbor's audit rows). All 9 products mount `/api/v1/audit-feed/` against their concrete `AuditLog`: admiralty (`foia/audit_feed.py`), beacon (`api/audit_feed.py`), bounty (`api/audit_feed.py`), harbor (`api/audit_feed.py`), helm (`api/audit_feed.py`), lookout (`api/audit_feed.py`), manifest (`signatures/audit_feed.py`), purser (`purser/audit_feed.py`), yeoman (`yeoman/audit_feed.py`). Reference template: `keel/feed/audit_feed_example.py`. Note that beacon's `AuditLog` (in `core/models.py:303`) doesn't inherit from `AbstractAuditLog` but has the same field shape — works fine because `audit_feed_view` reads fields by name and tolerates the missing `deep_link_snapshot` via `getattr(entry, 'deep_link_snapshot', '')`. Aggregator tolerates products without the endpoint: 404/ConnectionError surfaces as gray "pending" chip, 401/403 as "unauthorized", 5xx/JSON-parse as "error", timeout as "timeout". The page renders chip statuses before filter UI (incident-response scan order), and includes click-to-filter cells, deep-link entity columns via `deep_link_snapshot` (with `javascript:` / `data:` / protocol-relative URL allowlist), security-event row tints + jump pill, mobile filter drawer below 768px. Per-product result cap is 200 with a visible "200+" affordance on the chip. **Deployment requires `HELM_FEED_API_KEY` set on Keel's own Railway service** (same suite-wide value as the products) — without it, every chip shows "unauthorized." See "Django doesn't auto-import env vars" footgun below.
- **Railway "Wait for CI" gate — silent SKIPPED deploys.** All 9 services are wired to GitHub source with Railway's Source → Check Suites toggle ("Wait for CI") enabled (helm, manifest, harbor, admiralty, beacon, yeoman, lookout, bounty). When the GitHub Actions check suite fails on a commit, Railway records the deploy with `status=SKIPPED` and `skippedReason="CI check suite failed"` — no build logs, no error in the dashboard, the previous SUCCESS image keeps serving. `railway redeploy` redeploys the last successful image (still stale for code changes — but sufficient for env-var-only changes, since variables are injected at runtime); only `railway up` (uploads tarball, bypasses GitHub gate) ships new code. **Diagnose:** `railway deployment list --service <name> --json | jq '.[].meta.skippedReason'` — if the reason is "CI check suite failed", fix the failing CI step (it's almost always Bandit B602 on `subprocess.run(..., shell=True)` in `startup.py`, or a keel pin pointing at a rebased orphan SHA, or a Django app with a broken `import *` re-export). **A fourth cause: any non-CI workflow that runs against main's HEAD** — a scheduled or `workflow_dispatch` run (e.g. the Canary poller) stamps its check suite onto the HEAD commit, so a red canary blocks deploys of a commit whose actual CI is green. Bit admiralty + bounty 2026-07-22 during a metrics-token rotation (canary 403'd in the window before `railway redeploy` propagated the new token). Immediate fix: `gh run rerun <id> --failed` once the underlying condition clears — the re-run's conclusion replaces the failure in place. Structural fix (shipped 2026-07-22 to admiralty, bounty, helm canary.yml): the poll step uses `continue-on-error` and `notify` keys off the step outcome, so the canary workflow always concludes success and alerts only via the de-duped `canary`-labeled issue. New monitoring workflows MUST follow this shape — never let a prod-health check fail its check suite. Bounty's toggle is now ON (verified 2026-07-22 — an env-var deploy was SKIPPED on red CI and needed `railway redeploy`). Purser is believed to have the toggle off and ship even when CI is red — unverified since this note was first written.
- **`DEMO_PASSWORD` env var (added 0.19.0, removed 0.20.1).** keel 0.19.0's CSO security pass added a `_validate_demo_password` boot tripwire that required `DEMO_PASSWORD` to be set to a non-default value on every `DEMO_MODE=True` service, otherwise Django refused to import. The intent was to close the parallel `/accounts/login/` attack surface that the suite-wide `demo2026!` fallback opened, but the import-time crash crash-looped all 9 demo services on 2026-04-27 once the keel pin propagated faster than the env var did. 0.20.1 makes demo users passwordless (`set_unusable_password()`), so the credential, the env var, the `_INSECURE_DEMO_PASSWORD` sentinel, and the validator are all gone. **Action required when bumping a product to keel ≥ 0.20.1:** also update any product-specific seed commands (`seed_demo`, `seed_demo_users`, `seed_data.py`, etc.) to call `set_unusable_password()` instead of `set_password(DEMO_PASSWORD)`, and drop the `DEMO_PASSWORD` setting from `settings.py`. Otherwise the seed command silently writes a real Django hash on every demo user (using whichever fallback the product hardcoded — typically `demo2026!`), re-opening the form-login attack surface that this whole change exists to close. The `DEMO_PASSWORD` Railway env var is ignored after the bump and can be left in place or stripped.
- **Per-user inbox endpoint (`/api/v1/helm-feed/inbox/`) is the per-user companion to the aggregate helm-feed.** Powers Helm's "Awaiting Me" column. `keel.feed.helm_inbox_view` (added in 0.18.0) handles auth, rate limiting, per-user-per-path caching (NEVER cache by path alone — that would leak between users), and OIDC-sub → KeelUser resolution via `SocialAccount(provider='keel', uid=sub)`. Each peer wraps a `build_inbox(request, user) -> dict` function with `@helm_inbox_view`. Unknown sub returns 200 + empty `items[]` (not 404) so Helm's aggregator renders cleanly. Returns the `UserInbox` shape from `helm.dashboard.feed_contract`: `items[]` (id, type, title, deep_link, waiting_since, due_date, priority) + `unread_notifications[]`. **All 8 peers wired (2026-05-18):** Manifest (`signatures/helm_inbox.py` — signing steps), Harbor (`api/helm_inbox.py` — review + processing assignments), Admiralty (`foia/helm_inbox.py` — FOIA requests with statutory_deadline as due_date), Purser (`purser/helm_inbox.py` — submission reviews via Program.reviewers M2M), Bounty (`api/helm_inbox.py` — high-relevance OpportunityMatches), Lookout (`api/helm_inbox.py` — TrackedBills where user is principal driver in active stages), Beacon (`api/helm_inbox.py` — due KeepInTouch reminders + owned companies pending approval), Yeoman (`yeoman/helm_inbox.py` — open Invitations where user is assigned coordinator / principal / delegate).
- **`AbstractTag` and `AbstractGroup` adoption (added in 0.34.0).** Suite-wide tagging primitives now live in `keel.core.models`. Adopted products: **Beacon** (`companies.Tag` with `TagType` enum, `companies.ContactGroup` with `created_by.related_name='created_contact_groups'`); **Yeoman** (`yeoman.InvitationTag` with per-agency scoping). **Not yet adopted:** Bounty, Harbor, Lookout, Admiralty, Manifest, Purser, Helm. They have no tag/group needs today; reach for the abstracts when a product first wants to classify records. See the "Groups & Tags on Entity Records" section for the contract.
- **Suite-wide keel pin state (2026-05-14).** All 9 products pinned to `keel @ v0.41.0`. v0.41.0 contains the cross-product audit feed (`audit_feed_view` + `/audit/` page + `fetch_product_audit`), Dan's `KEEL_PRODUCT_NAME` / `KEEL_PRODUCT_CODE` split, the `HELM_FEED_API_KEY` settings wiring fix, and the v0.40.x notifications-inbox hook. The canonical `.dl-status-dot` rule still ships out of `docklabs-v2.css`; verification check unchanged from prior audits: `curl https://demo-<product>.docklabs.ai/static/css/docklabs-v2.*.css | grep -c dl-status-dot` should return **22** (was 17 when this note was written; the rule set grew — verified 22 suite-wide in the 2026-07-21 audit).
- **PR-after-tag keel releases hit the pip-cache trap.** `.dl-status-dot` was added in keel via PR #20 *after* v0.38.0 was tagged but `keel/__init__.py` was not bumped. Products whose pip wheel cache for `keel==0.38.0` was populated before PR #20 merged keep serving the pre-PR-#20 CSS forever (silent — no error, just missing styling). Bounty + Yeoman tripped on this during the 2026-05-13 design audit cycle. **Resolution:** cut a no-code-change keel v0.38.1 release to defeat the cache; bump the affected products. **Prevention:** the rule in "Keel version bumping — the pip cache trap" above isn't optional — *any* PR that changes shipping files (CSS, templates, Python) must bump `keel/__init__.py` + `pyproject.toml` in the same commit, not "before the next release."
- **Suite design audit (2026-05-11 → 2026-05-13) — what shipped.** See `/Users/dok/Code/CT/.gstack-suite-audit-20260513/SUITE-DESIGN-AUDIT.md` for the per-product delta. Headline: status-pill ➜ `.dl-status-dot` rule is now live suite-wide; Helm gained a 4-stat top-row strip; Manifest dashboard identity matches URL; Harbor sidebar flattened; Purser Program Status Grid rewritten as glyph+dot; Admiralty active-step callout borders stripped; Bounty opportunity titles in Fraunces serif; Yeoman invitations list demoted from 8 colored pills to dot+text (`needs_info` → exception pill). Reference page for detail-page layout is still Yeoman's `/invitations/<id>/`; reference for list-page status treatment is now Yeoman's `/invitations/`.
- **Two audit carry-forwards.** (1) CI smoke test that asserts `.dl-status-dot` is present in deployed `docklabs-v2.css` — would have caught the Bounty/Yeoman stale-wheel regressions at deploy time rather than via DOM inspection. (2) Yeoman's legacy `_status_badge.html` inclusion template is orphaned but kept; clean up in a follow-up. ~~(3) Local-only `claude/audit-feed-mount` branches on 7 product repos depend on `keel.feed.audit_feed_view`~~ — **landed 2026-05-14:** all 8 product PRs merged + deployed; keel `v0.40.1` tagged with the decorator; Keel `/audit/` page live. Only Beacon remains pending (see audit-feed pipeline entry above).
- **Django doesn't auto-import env vars into `settings`.** Setting an env var on a Railway service exposes it to the container as `os.environ['FOO']`, but Django's settings module only sees env vars it **explicitly reads** — i.e. `FOO = os.environ.get('FOO', '')` has to exist in `<service>_site/settings.py`. Code that does `getattr(settings, 'FOO', '')` will silently get the empty default otherwise. Discovered while shipping the audit-feed: Keel's Railway env had `HELM_FEED_API_KEY` set, but `keel_site/settings.py` didn't read it; the /audit/ chip row showed every product as "unauthorized" (lock icon) despite identical 43-char keys on both sides. Fix is one line (PR #25). **Review checklist:** any new `getattr(settings, '<UPPER_NAME>', ...)` in code MUST be paired with `<UPPER_NAME> = os.environ.get('<UPPER_NAME>', '')` in the consumer service's `settings.py`. Mirror the existing `KEEL_API_KEY = os.environ.get('KEEL_API_KEY', '')` pattern at `keel_site/settings.py:314`.
