...
- **Suite-wide keel pin state (2026-05-14).** All 9 products pinned to `keel @ v0.41.0`. v0.41.0 contains the cross-product audit feed (`audit_feed_view` + `/audit/` page + `fetch_product_audit`), Dan's `KEEL_PRODUCT_NAME` / `KEEL_PRODUCT_CODE` split, the `HELM_FEED_API_KEY` settings wiring fix, and the v0.40.x notifications-inbox hook. The canonical `.dl-status-dot` rule still ships out of `docklabs-v2.css`; verification check unchanged from prior audits: `curl https://demo-<product>.docklabs.ai/static/css/docklabs-v2.*.css | grep -c dl-status-dot` should return **17**.
